...
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────────┐  │
│  │lv-crypto │ │lv-format │ │lv-store  │ │lv-importer       │  │
│  │KDF·AEAD  │ │容器读写   │ │SQLite+   │ │1PUX/CSV/opvault  │  │
│  │CSPRNG    │ │序列化     │ │字段加密   │ │lv-exporter       │  │
│  └──────────┘ └──────────┘ └──────────┘ └──────────────────┘  │
│  ┌──────────┐ ┌──────────┐ ┌──────────────────────────────┐   │
│  │lv-totp   │ │lv-audit  │ │平台适配（trait 实现）         │   │
...
| 模块 | 职责 | 不做什么 | 平台相关 |
| --- | --- | --- | --- |
| `lv-crypto` | Argon2id KDF、XChaCha20-Poly1305 AEAD、CSPRNG、内存清零 | 不知道"条目"是什么 | 否 |
| `lv-format` | 容器头部读写、版本识别、格式迁移 | 不管理条目内容 | 否 |
| `lv-domain` | 条目/字段/保险库模型、22 类模板、校验 | 不碰 IO 与加密 | 否 |
| `lv-store` | SQLite schema、加密字段读写、事务、附件、历史版本 | 不做字段级业务校验 | 否 |
| `lv-importer` | 1PUX / CSV / opvault 解析、字段映射、预检报告 | 不直接写库（产出中间模型） | 否 |
| `lv-exporter` | 加密备份、1PUX 兼容导出、CSV 导出 | 不做 UI 提示 | 否 |
| `lv-totp` | TOTP 生成、otpauth URI 解析、QR 解码 | 不存储密钥（由 store 管） | 否 |
| `lv-audit` | 弱/重复/陈旧密码检测、弱 URL 检测、报告聚合 | 不联网（**硬约束**） | 否 |
| `lv-session` | 解锁/锁定、用例编排、权限门禁、空闲计时 | 不直接操作 UI | 否 |
| `lv-ffi` | 类型映射、错误映射、接口暴露 | 不含业务逻辑 | 否 |
...
| TOTP | 自研实现于 `lv-totp`，底层用 `hmac` + `sha1`/`sha2` | RFC 6238 逻辑简单，便于完全掌控；需标准测试向量验证 | 现成 TOTP crate（可评估，但需审计依赖） |
...
**门禁规则**：`lv-session` 是唯一持有 DEK 的模块。所有数据访问用例必须先通过 `require_unlocked()` 检查，未解锁时返回 `Error::VaultLocked`。**此检查在 Rust 侧强制执行，不能依赖 UI 层的状态管理**（UI 状态可被绕过，Rust 侧不行）。
...
目标 App 出现登录框
      │
      ▼
系统 Autofill Framework 回调 LocalVault 的 AutofillService
      │
      ▼
...
系统 Credential Manager 向已注册的凭据提供者发起查询
      │
      ▼
LocalVault 的 CredentialProviderService 被调用
...
（每个条目带 PendingIntent，**不含私钥**）
      │
      ▼
用户在系统选择器中选择 → PendingIntent 拉起 LocalVault 的 Activity
      │
      ▼
...
网站在 Safari 中请求创建或使用 Passkey
      ...
      ▼
系统调用已启用的凭据提供者扩展
（LocalVault 内的 App Extension，独立签名目标）
      │
      ▼
...
`lv-ffi` 是唯一对外边界。接口设计遵循"**粗粒度、面向用例**"——不为 `get_field()` 这种细粒度操作暴露接口，而是暴露"解锁保险库""搜索条目""执行导入"这类用例。理由：细粒度接口会导致 UI 侧大量往返调用，且难以保证事务与权限检查的一致性。
...
**错误处理**：所有接口返回 `Result<T, LvError>`，`LvError` 为带错误码的 enum。UI 侧按错误码做本地化提示，**不解析错误消息文本**。
...
```
UI: 用户输入主密码
  → lv-ffi: unlock_vault(vault_id, password)
  → lv-session: 取库文件头 → 读 salt 与 kdf_params
  → lv-crypto: Argon2id(password, salt, params) → KEK        [耗时 0.5–1.0s]
  → lv-crypto: XChaCha20-Poly1305 解封装 wrapped_dek → DEK
  → lv-crypto: 校验 verifier
  → lv-session: DEK 存入会话；启动空闲计时器
...
```
UI: 打开条目列表
  → lv-ffi: list_items(vault_id, filter?, sort?, page?)
  → lv-session: require_unlocked()
  → lv-store: SELECT uuid, category, updated_at, enc_title FROM items WHERE ...
...
```
UI: 选择文件 → lv-ffi: precheck_import(path)
  → lv-importer: 执行 ①~⑤
  → 返回 PrecheckReport（**不写库，无副作用**）
UI: 展示报告 → 用户确认策略
  → lv-ffi: execute_import(path, policy)
  → lv-session: BEGIN TRANSACTION
  → lv-importer: 执行 ③④
  → lv-store: 批量写入
...
| 威胁 | 场景 | 对策 |
| --- | --- | --- |
| **Spoofing 身份伪造** | 恶意 App 冒充 LocalVault 的填充界面骗取凭据 | 填充前强制身份校验；Android 侧用系统 Autofill 框架（系统保证调用方身份） |
