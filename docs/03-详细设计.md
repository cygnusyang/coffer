...
| v1.5 | 2026-09-29 | QA 修复批反向同步（以实现为准）：§2.6 verifier / wrapped_dek 的 AAD 修正为 `vault_uuid_bytes ‖ b"verifier"/b"wrapped_dek"`（docs/07 §2.2 裁定，钉死在库上防跨库重放，替代旧文常量 `b"cf/verifier/v1"`）；§3.3 搜索匹配规范明确为 **NFC 归一化 + 小写折叠**（查询串与标题两侧都做，防输入法混排的双形式 miss，与主密码归一化同构） |
...
**AAD 构造（v1.5 修订，以实现为准）**：verifier 与 wrapped_dek 的 AAD
**不是常量**，遵循 docs/07 §2.2 的裁定：

```
aad_verifier   = vault_uuid_bytes(16) ‖ b"verifier"
aad_wrapped_dek = vault_uuid_bytes(16) ‖ b"wrapped_dek"
```

```rust
const VERIFIER_PLAINTEXT: &[u8] = b"coffer-verifier-v1";   // 固定常量

// 创建时（vault_uuid 为本库的 uuid 的 16 字节原始值）
let (nonce, ct) = seal(&kek, aad = vault_uuid_bytes ‖ b"verifier", VERIFIER_PLAINTEXT);

// 解锁时
match open(&kek, nonce, ct, aad = vault_uuid_bytes ‖ b"verifier") {
    Ok(pt) if pt == VERIFIER_PLAINTEXT => UnlockOk,
    _ => Err(CfError::UnlockFailed),        // 密码错 与 数据损坏 均返回此码
}
```

**为什么 AAD 钉死 vault_uuid**（docs/07 §2.2 裁定，实现位于 `cf-session/src/unlock.rs`）：旧版常量 `aad = b"cf/verifier/v1"` 下，攻击者可以把**另一库**（密码相同或已攻破的库）的 `verifier` / `wrapped_dek` 密文整段替换进本库 header.json——常量 AAD 校验照样通过，被替换段在本库 KEK 下解封失败，只会归一为 1002（解锁失败），攻击无增益；但真正的风险在**反向场景**：攻击者持有库 A 的密钥材料时，可将 A 的 wrapped_dek 移植到库 B，配合 A 的密码即可让库 B 被「A 的凭据」解锁语义混淆，排查取证困难。AAD 钉死 vault_uuid 后，任何跨库搬运（verifier 或 wrapped_dek 单独搬、成对搬、同库内 verifier/wrapped_dek 段互搬——purpose 标签不同）一律 AEAD 认证失败归一 1002，密文在库维度被钉死。
...
fn search(&self, query: &str) -> Vec<ItemSummary> {
    // 匹配规范（v1.5）：NFC 归一化 + 小写折叠，查询串与标题两侧都做。
    // 与解锁流主密码的 NFC 归一化（cf-crypto::normalize_password）同构。
    let q = query.nfc().collect::<String>().to_lowercase();
    ...
    rows.filter_map(|r| {
        ...
        let normalized = title.nfc().collect::<String>().to_lowercase();
        if fuzzy_match(&normalized, &q) { Some(summary(r, title)) } else { None }
    }).collect()
}
```

**搜索匹配规范**（v1.5 明确）：**Unicode NFC 归一化 + 小写折叠**，查询串与条目标题**两侧都做**。理由：macOS 输入法与剪贴板混排时，视觉相同的字符串可能落成 NFC 与 NFD 两种字节序列（如 `é` = U+00E9 与 `e` + U+0301）；只折叠小写不归一化，NFD 标题用 NFC 查询（或反之）会 miss。这与解锁流对主密码做 NFC 归一化是同构的坑，两侧统一 NFC。
