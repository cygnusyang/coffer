...
| 形态 | 用途 | 路径示例 |
| --- | --- | --- |
| **工作形态（目录）** | App 日常读写 | Android: `/data/data/<pkg>/files/vaults/<vault_uuid>/`<br>macOS: `~/Library/Application Support/LocalVault/vaults/<vault_uuid>/` |
...
```
                    主密码 (UTF-8, NFC 归一化)
                              ...
                              ▼
              ┌───────────────────────────────┐
              │ Argon2id(m, t, p, salt, 32B)  │  耗时主体：目标 0.5–1.0 s
              └───────────────┬───────────────┘
                              ▼
                          KEK (32B)  ─── 不落盘 ───
                              │
                              │ XChaCha20-Poly1305 解封装
                              │ AAD = b"lv/dek/v1" ‖ vault_uuid
                              ▼
                          DEK (32B)  ─── 不落盤，会话持有 ───
                              │
                              │ HKDF-SHA256(salt = vault_uuid ‖ info)
                              ▼
   ┌──────────┬──────────┬──────────┬──────────┬──────────┬──────────┐
   ▼          ▼          ▼          ▼          ▼          ▼          ▼
meta_key  item_key  field_key  file_key  hist_key  manifest_key  attach_mac_key
...
**建议初值（待标定，非最终值）**：`m = 256 MiB, t = 3, p = 4`。
...
meta_key        = derive_subkey(dek, uuid, "lv/meta/v1");
item_key        = derive_subkey(dek, uuid, "lv/item/v1");
field_key       = derive_subkey(dek, uuid, "lv/field/v1");
file_key        = derive_subkey(dek, uuid, "lv/file/v1");
hist_key        = derive_subkey(dek, uuid, "lv/history/v1");
manifest_key    = derive_subkey(dek, uuid, "lv/manifest/v1");
attach_mac_key  = derive_subkey(dek, uuid, "lv/attach-mac/v1");
```
...
例：  aad = <items.uuid 的 16 字节> ‖ 0x00 ‖ b"enc_title"
```

**为什么这样构造**：攻击者若把 A 条目的 `enc_title` 密文复制到 B 条目的 `enc_title` 列，AAD 中的 UUID 不匹配 → 解密失败。若复制到 B 条目的其他列（如 `enc_value`），AAD 中的列名不匹配 → 同样失败。这使单条记录的密文在位置上被"钉死"。
...
```rust
const VERIFIER_PLAINTEXT: &[u8] = b"localvault-verifier-v1";   // 固定常量

// 创建时
let (nonce, ct) = seal(&kek, aad = b"lv/verifier/v1", VERIFIER_PLAINTEXT);

// 解锁时
match open(&kek, nonce, ct, aad = b"lv/verifier/v1") {
    Ok(pt) if pt == VERIFIER_PLAINTEXT => UnlockOk,
    _ => Err(LvError::UnlockFailed),        // 密码错 与 数据损坏 均返回此码
}
```
...
1. 在平台密钥库生成一个密钥 K_bio
   Android: KeyGenParameterSpec(alias="lv-bio-<vault_uuid>",
              setUserAuthenticationRequired(true),
              setInvalidatedByBiometricEnrollment(true))   ← 关键
   macOS:   SecAccessControlCreateWithFlags(
              kSecAttrAccessibleWhenUnlockedThisDeviceOnly,
              .biometryCurrentSet, ...)                     ← 关键
2. wrapped_dek_bio = XChaCha20-Poly1305(key = K_bio, aad = b"lv/bio/v1" ‖ uuid, pt = DEK)
...
fn search(&self, query: &str) -> Vec<ItemSummary> {
    ...
    rows.filter_map(|r| {
        let title = decrypt(item_key, &r.enc_title, aad(r.uuid, "enc_title"))?;
...
**`Designation` 设计要点**：`Other(String)` 分支让导入器能**原样保留** 1Password 中出现的任何 designation 字符串，导出时再原样写回。这实现了 **1PUX → LocalVault → 1PUX 的无损往返**。
...
通过 UniFFI 暴露。命名遵循 `动词_名词`，全部返回 `Result<T, LvError>`。
...
pub trait PlatformHost: Send + Sync {
    ...
    fn request_biometric(&self, reason: String) -> Result<(), LvError>;
    /// 请求写入剪贴板（含自动清除定时）
    fn copy_to_clipboard(&self, text: String, clear_after_secs: u32) -> Result<(), LvError>;
    /// 报告操作进度（导入/导出/重加密）
...
fn detect_format(path: &Path) -> ImportFormat {
    ...
    if head == b"PK\x03\x04" {
        ...
        if names.contains("header.json") { return ImportFormat::LocalVault; }
    }
...
③ 解析 export.data：
     accounts[] → 取第一个 account（或多 account 时逐库导入）
       vaults[] → 每个 vault 变为一个 LocalVault 库（或合并，见 §6.5）
         items[] → 逐条转换
④ 逐条转换：
...
> 来源：1Password 官方支持文档「OPVault design」，原文表述"These are three digit decimal numbers"，表中分类名与码值为官方给出。**表中 LocalVault 一列为本项目映射**。
...
| 4 | 映射表以**数据文件**（非硬编码）形式置于 `lv-importer/mapping/category_1pux.json`，可独立更新 |
...
| 1Password designation | LocalVault | 说明 |
| --- | --- | --- |
...
> 上表数值引自 RFC 6238 Appendix B。**实现完成后必须逐条跑通**，这是 `lv-totp` 的验收门槛（需求 AC-05）。若某条不通过，说明实现有 bug，不允许通过调整测试向量来"修好"。
...
> LocalVault **无法**检测"你的密码已在某次数据泄露中出现"。该能力需要查询在线的泄露库（如 Have I Been Pwned），而本产品不含任何网络能力。LocalVault 只做基于本地字典与规则的启发式判断。**如果你需要泄露检测，需要另行使用在线服务，或接受这一能力缺失。**
...
```kotlin
class LvAutofillService : AutofillService() {
...
```xml
<service
    android:name=".passkey.LvCredentialProviderService"
...
**"零网络"的 macOS 侧验证**：Hardened Runtime 下**不授予网络客户端 entitlement**，App 无法发起网络连接（系统层面拒绝）。用户可通过 `codesign -d --entitlements - LocalVault.app` 自行检查。
...
let query: [String: Any] = [
    ...
    kSecAttrService as String: "cn.localvault.biometric",
...
```
~/Library/Application Support/LocalVault/vaults/<vault_uuid>/
```
...
| **D-11** | macOS 凭据提供者扩展与主 App 的凭据传递机制 | 阻塞 macOS Passkey 实现 | 扩展只做代理，DEK 永不进扩展进程 |
